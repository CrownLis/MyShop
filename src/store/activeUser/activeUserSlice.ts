...
const activeUserSlice = createSlice({
  ...
  reducers: {
    ...
    decreaseAmountProduct(state, action: PayloadAction<number>) {
      state?.cart?.products.forEach((i) => {
        if (i.productId == action.payload && i.quantity > 1) {
          i.quantity--;
        }
      });
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(checkAuth.fulfilled, (state, action) => {
        (state.loading = false), (state.authKey = action.payload.authKey);
      ...
      .addCase(editUser.fulfilled, (state, action) => {
        (state.loading = false), (state.activeUser = action.payload);
      });
  }
});
