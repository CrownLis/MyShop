...
export const checkAuth = createAsyncThunk<
  ...
  Record<string, any>,
  { rejectValue: Error }
>('activeUser/logIn', async (values) => {
  const response = await logIn(values);
  const data = response.data;
  if (data !== 'username or password is incorrect') {
    const users = await fetchUsers();
    const activeUserData = users.data?.filter(
      (user: { username: any }) => user.username === values.username
    );
    const cart = await getUserCart(activeUserData[0].id);
    if (cart.data.length <= 0) {
      const newCart = await addNewCart({
        userId: activeUserData[0].id,
        date: new Date(),
        products: []
      });
      const activeUser = {
...
export const editUser = createAsyncThunk<
  ...
  [number, IUser],
  { rejectValue: string }
>('activeUser/editUser', async ([values, id]) => {
